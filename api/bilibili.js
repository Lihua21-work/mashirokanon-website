...
/**
 * Vercel / Serverless API 云函数
 * 部署于 Vercel/Netlify 时，服务端自动托管转发
 */
...
export default async function handler(req, res) {
  ...
  if (req && req.method === 'OPTIONS') {
    if (res && res.status) res.status(200).end()
    return
  }
  ...
  try {
    ...
    const navRes = await fetch('https://api.bilibili.com/x/web-interface/nav', { headers, signal: controller.signal })
    ...
    const dynRes = await fetch(dynUrl, { headers, signal: controller.signal })
